...
def run():
    ...
    for index, row in feature_df.iterrows():
        ...
        for frame in range(1, tdf_data.analysis["MaldiFrameInfo"].shape[0] + 1):
            scan_dict = {}
            frames_dict = tdf_data.analysis["Frames"][
                tdf_data.analysis["Frames"]["Id"] == frame
            ].to_dict(orient="records")[0]
            maldiframeinfo_dict = tdf_data.analysis["MaldiFrameInfo"][
                tdf_data.analysis["MaldiFrameInfo"]["Frame"] == frame
            ].to_dict(orient="records")[0]
            ...
            ook0_array = tims_scannum_to_oneoverk0(
                dll, tdf_data.handle, frame, range(0, frames_dict["NumScans"] + 1)
            )
            ...
            ook0_within_tolerance_scannums = tims_oneoverk0_to_scannum(
                dll,
                tdf_data.handle,
                frame,
                ook0_within_tolerance,
            )
            ook0_within_tolerance_scannums = [
                int(i) for i in ook0_within_tolerance_scannums
            ]
            ...
            scan_begin = 0
            scan_end = max(ook0_within_tolerance_scannums) - min(
                ook0_within_tolerance_scannums
            )
            for scan_num in range(scan_begin, scan_end):
                if (
                    ...
                    and list_of_scans[scan_num][0].size
                    == list_of_scans[scan_num][1].size
                ):
                    mz_array = tims_index_to_mz(
                        dll, tdf_data.handle, frame, list_of_scans[scan_num][0]
                    ).tolist()
                    ...
                    scan_dict["intensity"] += sum(
                        [intensity_array[i] for i in indices]
                    )
    ...
    results[["index", "integer"]] = results["Spot"].str.extract(
        "([A-Za-z]+)(\d+)", expand=True
    )

    group_columns = ["Frame", "Spot", "index", "integer"]
    results["numerator_intensity"] = results[
        results["ook0"] == args["numerator_ook0"]
    ].groupby(group_columns, as_index=False)["intensity"].transform("sum")
    results["denominator_intensity"] = results[
        results["ook0"] == args["denominator_ook0"]
    ].groupby(group_columns, as_index=False)["intensity"].transform("sum")
    
    if args["IS_mz"] is not None:
        results["IS_intensity"] = results[
            results["mz"] == args["IS_mz"]
        ].groupby(group_columns, as_index=False)["intensity"].transform("sum")

    if args["IS_mz"] is not None:
        results["n_numerator_intensity"] = (
            results["numerator_intensity"] / results["IS_intensity"]
        )
        results["n_denominator_intensity"] = (
            results["denominator_intensity"] / results["IS_intensity"]
        )
    ...
    if args["IS_mz"] is not None:
        grouped_results = results.groupby(
            ...
            [
                ...
                "IS_intensity",
                "n_numerator_intensity",
                "n_denominator_intensity",
            ]
        ].sum()
    else:
        grouped_results = results.groupby(
            ...
            ["numerator_intensity", "denominator_intensity"]
        ].sum()

    if args["IS_mz"] is not None:
        grouped_results["ratio"] = (
            grouped_results["n_numerator_intensity"]
            / grouped_results["n_denominator_intensity"]
        )
    else:
        grouped_results["ratio"] = (
            grouped_results["numerator_intensity"]
            / grouped_results["denominator_intensity"]
        )

    grouped_results.to_csv(
        os.path.join(args["outdir"], "modified_outfile.csv"), index=False
    )

    print(grouped_results)

    heatmap_data = (
        grouped_results[grouped_results["ratio"] != "-"]
        .groupby(["index", "integer"])["ratio"]
        .mean()
        .unstack()
    )
    ...
    heatmap_data.to_csv(
        os.path.join(args["outdir"], "heatmap_data.csv"), index=False
    )
