...
def get_args():
    ...
    parser.add_argument('--IS_mz',
                        help='Optional: m/z value from the feature list to be used for intensity normalization.',
                        type=float)
...
def run():
    ...
    for index, row in feature_df.iterrows():
        ...
        IS_mz = args['IS_mz'] if 'IS_mz' in args else None
        IS_tol = row['mz_tol'] if IS_mz is not None else None
	...
	#MaldiFrameInfo table in analysis.tdf SQL database tells which frame is associated with each spot    
        for frame in range(1, tdf_data.analysis['MaldiFrameInfo'].shape[0] + 1):
            scan_dict = {}
            frames_dict = tdf_data.analysis['Frames'][tdf_data.analysis['Frames']['Id'] == frame].to_dict(orient='records')[0]
            maldiframeinfo_dict = tdf_data.analysis['MaldiFrameInfo'][tdf_data.analysis['MaldiFrameInfo']['Frame'] == frame].to_dict(orient='records')[0]
            ...
            if IS_mz is not None and IS_tol is not None:
                scan_dict['IS_mz'] = IS_mz
                scan_dict['IS_tol'] = IS_tol
	    ...
	    #If 'IS_mz' is defined, the scan numbers for each spot and frame will be read for that feature, ignoring ooK0	
            if IS_mz is not None and IS_tol is not None:
                list_of_scans = tims_read_scans_v2(dll, tdf_data.handle, frame, 0, frames_dict['NumScans']+1)
            else:
                list_of_scans = tims_read_scans_v2(dll, tdf_data.handle, frame, min(ook0_within_tolerance_scannums), max(ook0_within_tolerance_scannums))
            for scan_num in range(len(list_of_scans)):
                if list_of_scans[scan_num][0].size != 0 and list_of_scans[scan_num][1].size != 0 and list_of_scans[scan_num][0].size == list_of_scans[scan_num][1].size:
		    ...
		    #Get indices of the m/z values within tolerance
                    indices = [mz_array.index(i) for i in mz_array if mz + mz_tol >= i >= mz - mz_tol]
                    scan_dict['intensity'] += sum([intensity_array[i] for i in indices])
                    if IS_mz is not None and IS_tol is not None:
                        is_indices = [mz_array.index(i) for i in mz_array if IS_mz + IS_tol >= i >= IS_mz - IS_tol]
			#This summed intensity is the intensity of of IS_mz +/- IS_tol 
    ...
    if 'IS_mz' in args:
        grouped_results['numerator_intensity'] = grouped_results['numerator_intensity'] / grouped_results['IS_intensity']
        grouped_results['denominator_intensity'] = grouped_results['denominator_intensity'] / grouped_results['IS_intensity']
